...
#             if i == idx:
#                 continue
#             negative_data_dict = self.dataset[i]
#             # negative_sentence = negative_data_dict['sentence']
#             # negative_encoded_input = self.tokenizer(negative_sentence, max_length=self.max_seq_length,
#             #                 add_special_tokens=True, truncation=True,
#             #                 padding='max_length', return_tensors="pt")
#             # build txt
#             negative_data_dict['txt_ids'] = data_dict['txt_ids']
#             negative_data_dict['txt_masks'] = data_dict['txt_masks']
#             # build object
#             negative_data_dict['obj_masks'] = (torch.arange(self.max_obj_len) < len(negative_data_dict['obj_locs'])) # O
#             negative_data_dict['obj_fts'] = self.pad_tensors(negative_data_dict['obj_fts'], lens=self.max_obj_len, pad=1.0).float() # O, 1024, 6
#             negative_data_dict['obj_locs']= self.pad_tensors(negative_data_dict['obj_locs'], lens=self.max_obj_len, pad=0.0).float() # O, 3
#             negative_data_dict['obj_boxes']= self.pad_tensors(negative_data_dict['obj_boxes'], lens=self.max_obj_len, pad=0.0).float() # O, 3
#             negative_data_dict['obj_labels'] = self.pad_tensors(negative_data_dict['obj_labels'], lens=self.max_obj_len, pad=-100).long() # O
#             # build sem mask, no mask
#             negative_data_dict['obj_sem_masks'] = (torch.arange(self.max_obj_len) < len(negative_data_dict['obj_locs']))
#             # build label for refer
#             # negative_data_dict['tgt_object_label'] = negative_data_dict['tgt_object_label'].long() # 1 or C
#             # negative_data_dict['tgt_object_id'] = negative_data_dict['tgt_object_id'].long() # 1 or O
#             # if len(negative_data_dict['tgt_object_id']) > 1: # O, pad to max objet length
#             #     negative_data_dict['tgt_object_id'] = self.pad_tensors(negative_data_dict['tgt_object_id'].long(), lens=self.max_obj_len, pad=0).long() # O
#             # build target
#             # if negative_data_dict.get('tgt_object_id_iou25') != None:
#             #     negative_data_dict['tgt_object_id_iou25'] = self.pad_tensors(negative_data_dict['tgt_object_id_iou25'], lens=self.max_obj_len, pad=0).long()
#             # if negative_data_dict.get('tgt_object_id_iou50') != None:
#             #     negative_data_dict['tgt_object_id_iou50'] = self.pad_tensors(negative_data_dict['tgt_object_id_iou50'], lens=self.max_obj_len, pad=0).long()
#             # # build label for qa
#             # if "answer_label" in negative_data_dict:
#             #     negative_data_dict['answer_label'] = negative_data_dict['answer_label'].long() # N, C
#             negative_data_dict_list.append(negative_data_dict)
#         data_dict['negative_data_dict'] = negative_data_dict_list
...
class ScanFamilyDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "prompt" in data_dict:
            data_dict['challenge'] = data_dict['challenge']
        return data_dict
