    ...
    def load_scannet(self, scan_ids, pc_type, load_inst_info):
        ...
        for scan_id in scan_ids:
            ...
            if pc_type == 'pred':
                ...
                obj_pcds = []
                obj_mask = np.array(sparse.load_npz(obj_mask_path).todense())[:78, :]
                obj_labels = np.load(obj_label_path)[:78]
...
class DataAugmentationMixin(object):
    ...
    def build_rotate_mat_no_random(self, angle_idx):
        # theta_idx = np.random.randint(len(ROTATE_ANGLES))
        angle_idx = int(angle_idx)
        theta = ROTATE_ANGLES[angle_idx]
