...
class Cross_Scene_Dataset(Dataset, LoadScannetMixin, DataAugmentationMixin):
    def __init__(self, split='train', max_obj_len=90, num_points=1024, pc_type='gt', sem_type='607', filter_lang=False, drop_sample=0.0):
        print("+++++++++++++++++++++  Use CrossScene-RETR ! +++++++++++++++++++++")
        ...
        # load file
        anno_file = './data/scanfamily/annotations/refer/newest_my_data_'+ split + '.jsonl'
        split_file = os.path.join(SCAN_FAMILY_BASE, 'annotations/splits/scene_id_'+ split + ".txt")
        ...
        print('Data lenght: '+ str(len(self.data)))
    ...
    def __getitem__(self, idx):
        ...
        if self.pc_type == 'gt':
           ...
           assert(self.int2cat[tgt_object_label] == tgt_object_name)
            ...
            if tgt_object_id >= len(self.scans[scan_id]["pcds"]):
                tgt_object_id = 0
        ...
        tgt_object_id_iou25 = torch.zeros(obj_fts.shape[0] + 1).long()
        tgt_object_id_iou50 = torch.zeros(obj_fts.shape[0] + 1).long()
...
if __name__ == '__main__':
    dataset = My_Data_Dataset(split='train', pc_type='gt')
    dataset[0]
    dataset[0]
    dataset[0]